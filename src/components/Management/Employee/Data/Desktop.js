...
const Desktop = () => {
    ...
    const loadUsers = async() => {
        const result = await axios.get("http://localhost:2020/desktop");
        setUser(result.data);
    };
                    ...
                    <tbody>
                        {
                            filteredList.map((user,index) => (
                                <tr>
                                    ...
                                    <td>{user.Experience.Network}</td>
                                    <td>{user.Experience.Tropologies}</td>
                                    <td>{user.Results.Linux}</td>
                                    <td>{user.Results.Network}</td>
                                    <td>{user.Results.Tropologies}</td>
                                    <td>{user.Salary}</td>
